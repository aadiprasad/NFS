...
int 
file_delete(struct trie *self, const char *key)
{
    if(trie_replace(self,key,identity,NULL)!=0)
    {
        return -1;
    }
    return 0;
...
//below is the only way I could figure out how to do this. The main issue is thta the function passed into visit can't have the trie in it. So I'm just noting down the full list and then deleting them.

struct chopping_block* die;

int
add_to_chopping_block(const char *key, void *data, void *arg)
{
    die->file_names[die->index] = calloc(strlen(key)+2,sizeof(char));
strcpy(die->file_names[die->index++],key);
    return 0;
}

int
trie_delete(struct trie *self, const char *key)
{
    int len = strlen(key);
    ...
    {
        printf("Deleting %s\n",die->file_names[i]);
        if(file_delete(self,die->file_names[i])!=0)
        {
            return -1;
        }
    }
    if(file_delete(self,key)!=0)
    {
        return -1;
    }
    for(int i=0;i<num_files;i++)
    {
        free(die->file_names[i]);
    }
    free(die->file_names);
    free(die);
    free(folder_path);

    // !Prune is only returning -1 for some reason. It's there for memory reasons. We can probably ignore it
    if(trie_prune(self)!=0)
    {
        return -1;
    }
    return 0;
...
/* (Slightly non-core visitor function?) */
static int
print_visitor(const char *key, void *data, void *arg)
{
    (void)arg;
...
{
    trie_visit(trie, "", print_visitor, NULL);
}

/* (Actual core starts here) */
static int
...
char **trie_keys(struct trie *trie, size_t *key_count) {
    ...
    *key_count = 0;
    ...
    while (!trie_it_done(it)) {
        if (it->data) {    
            ...
            keys[*key_count] = strdup(trie_it_key(it)); // Duplicate the key
