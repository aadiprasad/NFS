...
int trie_delete(struct trie *self, const char *key);

char **trie_keys(struct trie *trie, size_t *key_count);

int add_to_chopping_block(const char *key, void *data, void *arg);


struct chopping_block
{
    int index;
    char** file_names;
};

extern struct chopping_block* die;


extern struct trie * files; 
